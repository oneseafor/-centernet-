...
class SystemUi(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("智慧教室监控人数检测系统")
        self.setStyleSheet("QMainWindow{border: 1px solid #303030;}")
        # 左侧文本显示区
        self.function_label = QLabel(self)
        self.function_label.setFixedSize(219, 219)
        self.function_label.move(1, 1)
        ...
        self.function_label2.move(1, 430)
        ...
        self.btn4.clicked.connect(self.reset1_1)
        # 创建一个小区域
        ...
        self.conf_spin_box.setValue(self.identify_api.confidence)  # 当前值
        ...
        self.conf_slider.setValue(self.identify_api.confidence * 100)  # 当前值
        ...
        self.iou_spin_box.setValue(self.identify_api.nms_iou)  # 当前值
        ...
        self.iou_slider.setValue(self.identify_api.nms_iou * 100)  # 当前值
        ...
        self.result_label.move(778, 200)
        ...
        self.scroll_area.resize(200, 426)
        self.scroll_area.move(778, 233)
    ...
    def change_conf_spin_box(self):
        confidence = round(self.conf_spin_box.value(), 2)
        self.conf_slider.setValue(int(confidence * 100))
        self.identify_api.confidence = confidence

    # 滚动条改变检测置信度
    def change_conf_slider(self):
        confidence = round(self.conf_slider.value() * 0.01, 2)
        self.conf_spin_box.setValue(confidence)
        self.identify_api.confidence = confidence

    # 调节框改变检测交并比
    def change_iou_spin_box(self):
        nms_iou = round(self.iou_spin_box.value(), 2)
        self.iou_slider.setValue(int(nms_iou * 100))
        self.identify_api.nms_iou = nms_iou

    # 滚动条改变检测交并比
    def change_iou_slider(self):
        nms_iou = round(self.iou_slider.value() * 0.01, 2)
        self.iou_spin_box.setValue(nms_iou)
        self.identify_api.nms_iou = nms_iou
                ...
                if self.save_button_yes.isChecked():
                    ...
                    op_mysql.insert_one(
                        "insert into `records` VALUES ('"
                          + file_id + "','" + self.user_list[0] + "','" + file_name + "','" + file_time + "')")
                    # 创建一个消息框
                    msg_box = QMessageBox()
                    msg_box.setWindowTitle("提示")
    ...
    def camera_identify(self):
        if self.btn3.text() == "开启实时检测" and not self.timer_video.isActive():
            ...
            if flag is False:
                QMessageBox.warning(self, u"Warning", u"打开摄像头失败", buttons=QMessageBox.Ok, defaultButton=QMessageBox.Ok)
            else:
                self.timer_video.start(30)
                self.btn1.setDisabled(True)
        ...
        if self.output_image is not None:
            ...
            if self.save_button_yes.isChecked():
                ...
                if self.save_video_flag is False:
                    ...
                    op_mysql.insert_one(
                        "insert into `records` VALUES ('"
                          + file_id + "','" + self.user_list[0] + "','" + str(file_name) + "','" + file_time + "')")
    ...
    def show_table(self):
        ...
        for i in range(len(self.identify_record)):
            ...
            self.table_data(self.table_view, i, 1, self.identify_record[i]['record_name'])
            self.table_data(self.table_view, i, 2, self.identify_record[i]['record_time'])
    ...
    def select_record_by_date(self):
        ...
        if picker.exec_() == QDialog.Accepted:
            ...
            op_mysql = A_sql.OperationMysql()
            record_list = op_mysql.search("select * from records where record_user_id = '" + self.user_list[
                0] + "' and record_time like '" + date_result + "%';")
    ...
    def del_record(self):
        ...
        if len(select_list) == 0:
            ...
            for i in select_list:
                ...
                if i['record_name'][0:1] == 'c':
                    os.remove("./A_output/camera/" + i['record_name'])
                if i['record_name'][0:1] == 'i':
                    os.remove("./A_output/images/" + i['record_name'])
                op_mysql = A_sql.OperationMysql()
                op_mysql.delete_one("delete from records where record_time='" + str(i['record_time']) + "'")
            msg_box = QMessageBox()
            msg_box.setWindowTitle("提示")
            ...
            if self.date_show.text() == "--":
                ...
                self.identify_record = op_mysql.search("select * from records where record_user_id = '"
                                                       + self.user_list[
                                                           0] + "' and record_time like '" + self.date_show.text() + "%';")
        self.show_table()
    ...
    def show_admin(self):
        if len(self.user_list) != 0:
            self.admin_id2.setText(self.user_list[0])
            self.admin_pwd2.setText(self.user_list[1])

    def upd_password(self):
        if len(self.admin_pwd2.text()) != 0:
            if self.admin_pwd2.text() != self.user_list[1]:
                self.user_list[1] = self.admin_pwd2.text()
                op_mysql = A_sql.OperationMysql()
                op_mysql.update_one("update users set user_pwd='" + self.user_list[1] +
                                    "' where (user_id=  '" + self.user_list[0] + "')")
                # 创建一个消息框
                msg_box = QMessageBox()
                msg_box.setWindowTitle("提示")
